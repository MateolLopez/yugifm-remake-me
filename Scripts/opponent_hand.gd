...
func _check_exodia_deferred() -> void:
	...
	if bm == null:
		print("EXODIA DEBUG | OpponentHand: no encontró BattleManager en grupo battle_manager.")
		return

	if not ("draw_service" in bm):
		print("EXODIA DEBUG | OpponentHand: BattleManager no tiene draw_service.")
		return

	if bm.draw_service == null:
		print("EXODIA DEBUG | OpponentHand: bm.draw_service es null.")
		return

	if not bm.draw_service.has_method("check_exodia_after_draw"):
		print("EXODIA DEBUG | OpponentHand: draw_service no tiene check_exodia_after_draw().")
		return

	await bm.draw_service.check_exodia_after_draw("Opponent")
